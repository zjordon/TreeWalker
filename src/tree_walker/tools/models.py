...
ACTION_DEFINITIONS: dict[str, tuple[type[BaseModel], str, bool]] = {
    ...
    "read_grid": (
        ...
        (
            ...
            "Rows come back keyed by COLUMN HEADER, so adjacent numeric columns cannot "
            "be confused — use this instead of reading table values off the DOM "
            "snapshot by cell position. When the table has a Total/合计 row, it is "
            "returned as footer together with computed per-column sums "
