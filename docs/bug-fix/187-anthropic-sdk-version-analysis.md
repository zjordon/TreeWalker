...
2. `uv lock` 刷新至 **0.122.0**——理由（v2 修订）：0.x 线内较新版本（2026-08-13），非"与 eval 实证环境对齐"（eval 从未在 0.122 跑过，见 §6）；不追 0.125 仅取保守。**实施落点（2026-09-15）：`uv lock --upgrade-package anthropic` 定向解析自然取 0.x 末版 0.125.0（2026-08-19）**——与偏好 0.122 的差异不影响诉求（两者 temperature ✅、同 0.x 线，契约由 `<1.0` 上界保证而非精确版本），实施方案 §2 已按此决策；
