"""Example: 用 skill 加持的 agent 在 B 站发视频，并录制成可重放历史。

相比 ``examples/upload_file_bilibili.py``，本示例多启用两个开关：
  ...
  1. **skill 开关**（默认关闭，这里显式开启）
     ...
     - 双层静默：目录/文件不存在时不报错、不注入（零行为变更）。所以没有 skill
       文件也能跑——但建议准备 ``domain-skills/www.bilibili.com/{selectors.md,quirks.md}``
       才能真正看到 skill 的收益。
...
Usage:
    ...
    uv run python examples/skill/upload_bilibili_with_skill.py
"""
...
async def main():
    ...
    cover_path = r"D:\dev\git\claude\skills-deom\ppt\browser-use\横封面.png"
    ...
    agent_settings = AgentSettings(
        ...
        allowed_upload_paths=[video_path, cover_path],
    ...
    # ── ② 录制：把探索历史存成可重放 JSON ──
    print("\n=== 录制历史 ===")
    history_file = "bilibili_upload_skill.json"
